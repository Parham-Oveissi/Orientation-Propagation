...
for k = 1:length(time)
    Orientation_Matrix_M2 = expm(-super_cross(omega)*dt)*Orientation_Matrix_M2;
    % Orientation_Matrix_M2 = expm(-super_cross(omega(:,k))*dt)*Orientation_Matrix_M2;
    Orientation_Matrix_M2_total(:,:,k+1) = Orientation_Matrix_M2;

    figure(5)
    Animate_Orientation_Matrix(Orientation_Matrix_M2, 1e-1)
end
